...
public class main {
	...
	public static void main(String[] args) {
		...
		Buque bB = new Buque();
		  ...
		  System.out.println("Tiempo recorrido" + circuitoA.tiempoRecorridoEntre(Argentina,España)); 
		  ...
		  //A partir de los buques que tiene el circuito, y sus fechas de salida,
		...
		And = new And();
		Filtro fSimple = new PuertoDestino(Brasil);
		Filtro fSimple2 = new PuertoDestino(España);
		...
		for (List<Tramo> tList : b.buscar()) {
			tList.forEach(t -> System.out.print(
					"Origen :" + t.getOrigen().getNombre() + " |Destino :" + t.getDestino().getNombre() + " - "));
			System.out.println();
		}

		/*
		 * Argentina = new Terminal("Argentina", new UbicacionGeografica(19,20)); Brasil
		 * = new Terminal("Brasil", new UbicacionGeografica(1,4)); España = new
		 * Terminal("España", new UbicacionGeografica(500,320)); China = new
		 * Terminal("China", new UbicacionGeografica(1400, 500));
		 * 
		 * lineaA = new Naviera();
		 * 
		 * circuitoA = new CircuitoMaritimo(Argentina, España); circuitoB = new
		 * CircuitoMaritimo(Argentina, China);
		 * 
		 * // Circuito A = [Argentina, Brasil, España] circuitoA.agregarTramo(Argentina,
		 * Brasil, 4); circuitoA.agregarTramo(Brasil, España, 20);
		 * circuitoA.agregarTramo(España, Argentina, 22.3d);
		 * 
		 * //Circuito B = [Argentina, España, China] circuitoB.agregarTramo(Argentina,
		 * España, 4); circuitoB.agregarTramo(España, China, 30d);
		 * circuitoB.agregarTramo(China,Argentina, 55d);
		 * 
		 * 
		 * 
		 * bA = new Buque(); bB = new Buque();
		 * 
		 * lineaA.agregarCircuitoMaritimo(circuitoA);
		 * lineaA.agregarCircuitoMaritimo(circuitoB); lineaA.agregarBuque(bA);
		 * lineaA.agregarBuque(bB);
		 * 
		 * 
		 * //Argentina.asignarViaje(bB, circuitoA); lineaA.asignarViaje(bA, circuitoA,
		 * LocalDateTime.of(LocalDate.of(2025,10,31), LocalTime.of(1, 0)));
		 * lineaA.asignarViaje(bB, circuitoB, LocalDateTime.of(LocalDate.of(2025,12,1),
		 * LocalTime.of(23, 0)));
		 * 
		 * //lineaA.showCronogramaBuque();
		 * 
		 ...
		 * System.out.println(c.getTerminalOrigen().getNombre() + " - " +
		 * c.getTerminalDestino().getNombre() + " | Duracion: " +
		 ...
		 * 
		 * 
		 * Circuito A = [Argentina, Brasil, España] Circuito B = [Argentina, España,
		 * China]
		 * 
		 */
	}
